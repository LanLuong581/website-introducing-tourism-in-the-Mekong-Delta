...
ob_start();
$tinh_id = $_GET["tinh"];
$loaitour_id = $_GET["loaitour"];
$sql_tinh = "SELECT _name FROM `province` WHERE id='$tinh_id'";
$result_tinh = mysqli_query($conn, $sql_tinh);
if (mysqli_num_rows($result_tinh) > 0) {
    while ($result = mysqli_fetch_assoc($result_tinh)) {
        $ten_tinh = $result['_name'];
    }
}

$sql_loaitour = "SELECT `loaitour_id`, `loaitour_ten` FROM `loai_tour` WHERE loaitour_id='$loaitour_id'";
...
if (mysqli_num_rows($result_loaitour) > 0) {
    while ($loaitour = mysqli_fetch_assoc($result_loaitour)) {
        $ten_loai = $loaitour['loaitour_ten'];
    }
}
        ...
        <div class="main_content" style="background-color: #f8f9fa;padding-top:50px">
            <form action="" method="POST">
                ...
                FROM (((((( chitiet_tour
                      ...
                      WHERE account.daily_trangthai='1' && tour.tour_gioithieu != 'NULL' && tour.tinh_id=$tinh_id && loai_tour.loaitour_id='$loaitour_id'";
                        $result_tour = mysqli_query($conn, $sql_tour);
                        if (mysqli_num_rows($result_tour) > 0) {
                        ?>
                            <h4>Tìm thấy <?php echo mysqli_num_rows($result_tour) ?> kết quả cho "<?php echo $ten_tinh ?>" thuộc loại "<?php echo $ten_loai?>"</h4>
                            <?php
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-8" style="text-align: center;">
                                        <div class="empty-cart" style="background-color: #fff;height:200px;margin-bottom:10%;padding-top:20px;padding-bottom:20px">
                                            <h5>Không tìm thấy kết quả nào cho "<?php echo $ten_tinh ?>" thuộc loại "<?php echo $ten_loai?>"</h5>
