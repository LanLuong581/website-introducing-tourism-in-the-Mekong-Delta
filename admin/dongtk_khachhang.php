...
//đóng tài khoản doanh nghiệp bằng cách update khachhang_trangthai =1
$account_id = $_GET['account_id'];
$sql_update = "UPDATE `account` SET `khachhang_trangthai`='0' WHERE  `account_id`='$account_id'";
if (mysqli_query($conn, $sql_update)) {
    // gửi mail cho khach hang khi đóng
    ...
    $mail->Subject = "<ViVuMekong> Đóng tài khoản";
    $mail->setFrom("thanhlan581@gmail.com");
    $mail->Body = "Xin chào $account_email,<br>
                Tài khoản của bạn trên ViVuMekong đã bị đóng do bạn đã vi phạm điều khoản sử dụng<br>
                Vui lòng liên hệ ViVuMekong để được mở lại tài khoản<br>
                Xin cám ơn!
                ";
    $mail->addAddress($account_email);
    $mail->isHTML(true);
    if ($mail->send()) {
        header('location:khachhang.php?b=1');
    } else {
